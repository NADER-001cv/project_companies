...
<div class="card z-index-0 fadeIn3 fadeInBottom">
    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
      <div class="bg-gradient-primary shadow-primary border-radius-lg py-3 pe-1">
        <h4 class="text-white font-weight-bolder text-center mt-2 mb-0">Add new job</h4>
        
      </div>
    </div>
    <div class="card-body" style="margin-top: 50px">
      <form  action="{{route('jobs.update',$job)}}" method="POST" enctype="multipart/form-data" role="form" class="text-start">
        @method('PUT')
        @csrf
        ...
        <option value="{{$categorie->id}}" {{ $job->category_id == $categorie->id ? 'selected' : '' }} >{{$categorie->name }} </option>
        ...
        <div class="file-field col s12">
          ...
          <img  src=" {{ asset('storage/'.$job->gallery) }}"style=" max-width:30%; max-height: 227px;" alt="hhdkjx">
